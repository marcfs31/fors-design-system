...
description: >-
  ...
  DOM-snapshot, token-contrast, the Storybook Vitest addon, built-artifact smoke,
...
| **Storybook Vitest addon** — `@storybook/addon-vitest`, wired in `vitest.config.ts` + `.storybook/preview.ts` | `npm run test:storybook` (watch mode) or `npm run test:storybook:ci` (one-shot) | real headless **Chromium** via `@vitest/browser` + Playwright | (1) **render smoke on every story** — any story that throws on mount fails, even states no unit test covers; (2) **full axe per story** including `color-contrast`, which jsdom cannot evaluate; (3) **`play` interaction tests** — real-browser user flows |
...
CI runs the unit/coverage/smoke/package/size layers on Node 20 and 22, and
the Storybook Vitest addon as a separate job.
...
3. **`play` interaction test** on the story for any user flow the unit test can't drive well in jsdom — overlay open/close, keyboard navigation, a form submit path. Put it on a dedicated story (`KeyboardNavigation`, `OpenAndClose`, …) using `expect` / `userEvent` / `within` from `storybook/test`. The Storybook Vitest addon executes it in a real browser.
...
6. **New story** → nothing extra: the Storybook Vitest addon picks it up automatically for render smoke + axe.
...
npm run test:storybook       # watch mode; auto-starts `storybook dev` if it isn't already running
```
