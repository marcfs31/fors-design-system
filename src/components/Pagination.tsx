  ...
  ({ className, active, ...props }, ref) => (
    <button
      ...
      className={cn(
        ...
        active ? "bg-accent text-accent-fg" : "text-fg-secondary hover:bg-ink-surface-2 hover:text-fg",
...
export function PaginationEllipsis({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) {
  return (
    <span
      aria-hidden="true"
      className={cn("flex h-9 w-9 items-center justify-center font-sans text-sm text-fg-muted", className)}
      {...props}
    >
      …
    </span>
