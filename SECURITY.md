...
The **source** of this project is public. The **package** (`@marcfs31/fors-design-system`) is not — it's published to GitHub Packages with `restricted` access and installed only by Marc Fors's own repos, so there is no broad public consumer base.
